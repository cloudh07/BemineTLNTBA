...
type StoredListener = AppEventListener;

@injectable()
export class EventsService {
  eventSubscribers: {[key: string]: Set<StoredListener>} = {};
  ...
  subscribe<T>(event: string, listener: AppEventListener<T>) {
    ...
    this.eventSubscribers[event].add(stored);
    console.log(
    ...
    return {
      unsubscribe: () => {
        this.unsubscribe(event, stored);
      },
    };
  }

  private unsubscribe(event: string, listener: StoredListener) {
