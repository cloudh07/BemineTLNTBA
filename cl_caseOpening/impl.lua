function CacheWithGetter(getter)
	local cache = {}
	setmetatable(cache, {
...
---@field implCache table
---@field intervals table
---@field LogInfo fun(self: self, msg: string, ...: any)
---@field LogError fun(self: self,msg: string, ...: any)
---@field LogSuccess fun(self: self,msg: string, ...: any)
---@field LogWarning fun(self: self,msg: string, ...: any)
---@field Interval fun(self: self, name: string, ms: number, cb: fun(breakFn:fun()), done?:fun())
---@field ClearInterval fun(self: self, id: string)
Class = {}
...
function Class:extend(obj)
	...
	obj.implCache = CacheWithGetter(function(name)
		local impl = main:GetImpl(name)
		if not impl then
			error(("Impl %s not found"):format(name))
		end
		return impl
	end)
	if not obj.LogInfo then
		function obj:LogInfo(msg, ...)
			main:LogInfo("[^9class " .. self.name .. "^0] " .. msg, ...)
		end

		function obj:LogError(msg, ...)
			main:LogError("[^9class " .. self.name .. "^0] " .. msg, ...)
		end

		function obj:LogSuccess(msg, ...)
			main:LogSuccess("[^9class " .. self.name .. "^0] " .. msg, ...)
		end

		function obj:LogWarning(msg, ...)
			main:LogWarning("[^9class " .. self.name .. "^0] " .. msg, ...)
		end
	end
	obj.intervals = {}
	function obj:Interval(name, ms, fn, done)
		local id = GenerateUUID()
		self.intervals[id] = true
		local breakFn = function()
			self.intervals[id] = false
		end
		Citizen.CreateThread(function()
			self:LogInfo("Starting interval %s %s", name, id)
			while not self.destroyed and self.intervals[id] do
				fn(breakFn)
				Wait(ms)
			end
			self:LogInfo("Stopping interval %s %s", name, id)
			if done then done() end
		end)
		return id
	end

	function obj:ClearInterval(id)
		self.intervals[id] = false
	end
...
function Impl:ReplaceMethod(method, newMethod)
	...
	self:LogInfo("Replacing method %s", method)
...
function Impl:OnResourceStop()
	self:LogWarning("Not implemented OnResourceStop")
end
...
function Impl:LogInfo(msg, ...)
	main:LogInfo("[^6impl " .. self.name .. "^0] " .. msg, ...)
end

function Impl:LogError(msg, ...)
	main:LogError("[^6impl " .. self.name .. "^0] " .. msg, ...)
end

function Impl:LogSuccess(msg, ...)
	main:LogSuccess("[^6impl " .. self.name .. "^0] " .. msg, ...)
end

function Impl:LogWarning(msg, ...)
	main:LogWarning("[^6impl " .. self.name .. "^0] " .. msg, ...)
end
