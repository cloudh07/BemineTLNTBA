...
AddEventHandler("onResourceStop", function(resourceName)
  if resourceName == GetCurrentResourceName() then
    main:LogInfo("Stopping resource")
    for k, v in pairs(main.initializedImpls) do
      v:OnResourceStop()
    end
  end
end)
...
Citizen.CreateThread(function()
  ...
  if not IsDuplicityVersion() then
    ...
    if Config.Framework == 'esx' then
      local esxObj = Framework
      Framework = {}
      local cache = {}
      local hGetPlayerFromId = function(playerSrc, notCache)
        -- if not cache[playerSrc] then
        --   cache[playerSrc] = esxObj.GetPlayerFromId(playerSrc)
        -- end
        -- if notCache then
        --   cache[playerSrc] = esxObj.GetPlayerFromId(playerSrc)
        -- end
        return esxObj.GetPlayerFromId(playerSrc)
      end
      local hGetPlayerFromIdCard = function(id)
        for _, v in pairs(cache) do
          if v.id == id then
            return v
          end
        end
        local xPlayer = esxObj.GetPlayerFromIdCard(id)
        if xPlayer then
          cache[xPlayer.source] = xPlayer
          return xPlayer
        end
        return nil
      end
      local hGetPlayerFromIdentifier = function(identifier)
        -- for _, v in pairs(cache) do
        --   if v.identifier == identifier then
        --     return v
        --   end
        -- end
        local xPlayer = esxObj.GetPlayerFromIdentifier(identifier)
        if xPlayer then
          -- cache[xPlayer.source] = xPlayer
          return xPlayer
        end
      ...
      setmetatable(Framework, {
        __index = function(self, k)
          if k == "GetPlayerFromId" then
            return hGetPlayerFromId
          elseif k == "GetPlayerFromIdcard" then
            return hGetPlayerFromIdCard
          elseif k == "GetPlayerFromIdentifier" then
            return hGetPlayerFromIdentifier
          else
            return esxObj[k]
          end
        end
...
local function CacheWithGetter(getter)
  local cache = {}
  setmetatable(cache, {
...
local implCache = CacheWithGetter(function(name)
  local realImplName = name
  if IsDuplicityVersion() then
    realImplName = name:match("^SV(.-)Impl$")
  else
    realImplName = name:match("^CL(.-)Impl$")
  end
  return main:GetImpl(realImplName)
end)
---@generic T
---@param impl `T`
---@return T
